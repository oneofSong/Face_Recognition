...
class faceRecognition():
    ...
    def rgbResize(self, image, sizeX=None, sizeY=None):
        ...
        if image is None:
            ...
            self.imgHeight, self.imgWidth, self.imgChanel = image.shape
        ...
        print("이미지 변환 전 imgHeight : {}, imgWidth : {}, imgChanel : {}".format(self.imgHeight, self.imgWidth, self.imgChanel))

        # image 의 사이즈 측정
        # image 의 사이즈가 Default size 300x300 보다 크면 300x300 으로 축소
        # image 의 사이즈가 Default size 300x300 보다 작으면 300x300 으로 확대
        if self.imgHeight > 300 and self.imgWidth > 300:
        ...
        self.imgHeight, self.imgWidth, self.imgChanel = image.shape
        print("이미지 변환 후 imgHeight : {}, imgWidth : {}, imgChanel : {}".format(self.imgHeight, self.imgWidth, self.imgChanel))
    ...
    def findFaceInImg(self, imgFilePath=None, img=None):
        ...
        for idx, faceRect in enumerate(detectedFaces):
            ...
            coordDataList.append(faceRect)          # 검출 얼굴 영역 좌표점
            coordDataList.append(poseLandmarks)     # 랜드마크 데이터 입력
        ...
        print("coordDataList :: (얼굴 / 랜드마크 검출 결과)")
        print(coordDataList)
